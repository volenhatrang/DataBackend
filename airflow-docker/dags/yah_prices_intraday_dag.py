...
def check_updated_tickers():
    ...
    table_name = "download_yah_prices_intraday_history"
    updated_tickers = []
...
def fetch_yah_prices_intraday_by_list(list_codesource, group_id, batch_size=50):
        ...
        try:
            ...
            def fetch_single_ticker(ticker):
                ...
                for attempt in range(retries):
                    try:
                        df = download_yah_prices_intraday_by_code(
                            ticker, pInterval="5m"
                        )
                ...
                for i, ticker in enumerate(list_codesource):
                    ...
                    if len(all_data) >= batch_size or i == len(list_codesource) - 1:
                        if all_data:
                            ...
                            # Chèn batch vào Cassandra (nếu cần)
                            # cassandra_client.batch_insert_prices_async(final_df, table_name, 100)

                            # logging.info(
                            #     f"Processed batch of {len(final_df)} rows for {group_id}"
                            # )
                            if final_df:
                                timestamp_columns = [
                                    "date",
                                ...
                                data_crawled = json.dumps(
                                    final_df.to_dict(orient="records")
                                )

                                cassandra_data = {
                                    "url": "Yahoo Finance",
                                    "title": "yahoo_intraday",
                                    "crawl_date": datetime.now(),
                                    "content": "Download Data Intraday YAH 5m",
                                    "data_crawled": data_crawled,
                                }

                                existing_data = (
                                    cassandra_client.fetch_latest_data_by_title()
                                )

                                if (
                                    existing_data
                                    and cassandra_data["title"] in existing_data
                                ):
                                    existing_crawled = json.loads(
                                        existing_data["data_crawled"]
                                    )
                                    new_crawled = json.loads(data_crawled)
                                    combined_crawled = existing_crawled + new_crawled
                                    cassandra_data["data_crawled"] = json.dumps(
                                        combined_crawled
                                    )

                                cassandra_client.insert_raw_data(cassandra_data)

                                logging.info(
                                    f"Inserted complete JSON for {group_id} with {len(final_df)} rows"
                                )
                            del final_df
                            all_data = []
...
#             python_callable=fetch_yah_prices_intraday_by_list,
