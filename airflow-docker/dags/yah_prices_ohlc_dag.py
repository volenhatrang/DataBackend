...
def check_and_mark_group(group_id, reset=False):
    try:
        ...
        Variable.set("processed_groups", "{}")  # Reset lại giá trị bị lỗi
        return False
...
def get_list_tickers(nb_group=3, updated_tickers=None):
    ...
    if nb_group is None or nb_group <= 1:
        return tickers
...
with DAG(
    ...
    chunks = [list_codesource[i : i + 100] for i in range(0, len(list_codesource), 100)]

    with TaskGroup("fetch_yahoo_prices_group") as fetch_task_group:
        fetch_tasks = []
